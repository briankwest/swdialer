...
- `frontend/` — Vite + React + TypeScript on port **5173**. All WebRTC (signaling, media, DTMF) goes through the **published npm package** `@signalwire/js` (pinned to the dev build `4.0.0-dev-20260911205444` in `frontend/package.json`, for testing ahead of the next RC; previously `4.0.0-rc.0`) directly from the browser. There is **no** local SDK alias — `vite.config.ts` resolves `@signalwire/js` from `node_modules` like any other dependency (and pre-bundles it plus `rxjs` via `optimizeDeps`). npm's `latest`/`rc`/`dev` tags move often; check `npm view @signalwire/js dist-tags` rather than assuming. Pin prereleases **exactly** (`npm install --save-exact`) — a caret range on a prerelease will float to other prereleases.
- `signalwire-js/` — a local, gitignored checkout (its own nested git repo) of the SignalWire Browser SDK monorepo at `4.0.0-rc.0` — **older than the pinned dev build**, so its source can lag what actually ships in `node_modules/@signalwire/js/dist`. It is **not** wired into the dialer build (we consume the npm package); keep it for reference only — its `examples/` directory is the authoritative guide to the v4 API, and `packages/main/src/` is the SDK source if you need to read it. (It replaced the old `browser-sdk/` alias, which is gone.)
